...
class Accounts
{
    ...
    public function add($adminuser,$username,$password,$email,$timezone) 
    {
        ...
        // Copy balance across from linked user
        $linkeduser_balance = $this->redis->hget("billing:$linkeduser","balance");
        $adminuser_balance = $this->redis->hget("billing:$adminuser","balance");
        $this->redis->hset("billing:$adminuser","balance",$adminuser_balance+$linkeduser_balance);
        $this->redis->hset("billing:$linkeduser","balance",0);
    ...
    public function unlink($adminuser,$linkeduser)
    {
        ...
        $balance = $this->redis->hget("billing:$adminuser","balance");
        if ($balance<0) {
            return array ("success"=>false, "Accounts cannot be unlinked while balance is negative");
        } 
